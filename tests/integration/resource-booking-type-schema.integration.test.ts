...
async function assertLongLocaleSupport(pocketbase: PocketBase, tenantId: string) {
  const longLocale = "zh-Latn-CN-variant1-variant2-variant3";
  const updatedTenant = await pocketbase
    .collection("tenants")
    .update(tenantId, { locale: longLocale });
...
it("enforces currency, locale, safe rates, and normalized tenant uniqueness", async () => {
  ...
  await assertLongLocaleSupport(pocketbase, tenant.id);
